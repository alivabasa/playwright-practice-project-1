...
test.describe("Login UI Tests", async()=>{
    ...
    test("TC4: User logs in with wrong email", async({page})=>{
        ...
        await loginPage.goToLoginPage(page);
        await expect(loginPage.loginFormText).toBeVisible();
        await loginPage.loginWithCredentials("abc1234@com",user.password);
        await expect(loginPage.loginFormErrorMessage).toBeVisible();
        await expect(loginPage.loginFormErrorMessage).toHaveText(
            loginPage.INVALID_CREDENTIALS_ERROR_MESSAGE
        );
    ...
    test("TC5: User logs in with wrong password", async({page})=>{
        ...
        await loginPage.goToLoginPage(page);
        await expect(loginPage.loginFormText).toBeVisible();
        await loginPage.loginWithCredentials(user.email,"invalidPassword");
        await expect(loginPage.loginFormErrorMessage).toBeVisible();
        await expect(loginPage.loginFormErrorMessage).toHaveText(
            loginPage.INVALID_CREDENTIALS_ERROR_MESSAGE
        );
    ...
    test("TC7: User logs in with invalid email format", async({page})=>{
        ...
        await loginPage.goToLoginPage(page);
        await expect(loginPage.loginFormText).toBeVisible();
        await loginPage.loginWithCredentials("mkl.com",user.password);
        const validationMessage = await loginPage.emailAddresInput.evaluate(
            (el: HTMLInputElement) => el.validationMessage
        );
    ...
    test("TC8: User logs in with valid credentials successfully & validates the dashboard", async({page})=>{
        ...
        expectedNavbarItems.push(` Logged in as ${user.name}`);
